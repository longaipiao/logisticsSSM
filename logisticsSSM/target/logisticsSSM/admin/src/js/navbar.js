...
layui.define(['layer', 'laytpl', 'element'], function(exports) {
  ...
  var navbar = {
    ...
    bind: function(callback, params) {
      ...
      _target.find('a.js[kit-target]').each(function() {
        var _that = $(this),
          tipsId = undefined;
    ...
    render: function(callback) {
      var that = this,
        ...
        _tpl = [
          ...
          '<dd>',
          '<srgall.js href="javascript:;" kit-target data-options="{url:\'{{child.url}}\',icon:\'{{child.icon}}\',title:\'{{child.title}}\',id:\'{{child.id}}\'}">',
          ...
          '</dl>',
          '{{# }else{ }}',
          '<srgall.js href="javascript:;" kit-target data-options="{url:\'{{item.url}}\',icon:\'{{item.icon}}\',title:\'{{item.title}}\',id:\'{{item.id}}\'}">',
          '{{# if (item.icon.indexOf("fa-") !== -1) { }}',
          '<i class="fa {{item.icon}}" aria-hidden="true"></i>',
          '{{# } else { }}',
          '<i class="layui-icon">{{item.icon}}</i>',
